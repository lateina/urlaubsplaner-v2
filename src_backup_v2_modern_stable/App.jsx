...
const App = () => {
  ...
  const handleLogin = (loginData) => {
    ...
    loadData(loginData.masterKey);
  };

  const handleLogout = () => {
    if (!confirm('Abmelden?')) return;
    setAuth({ user: null, masterKey: '', isAuthenticated: false });
  ...
  const isAdmin = auth.user?.role === 'admin' || auth.user?.id === 'admin' || auth.user?.id === 'sekretariat';
  ...
  const renderContent = () => {
    ...
    switch (activeTab) {
      ...
      case 'summary':
        return (
          <div style={{ flex: 1, overflow: 'auto', padding: '24px', background: 'white' }}>
            <AbsenceSummary 
              employees={appData.employees} 
              absences={appData.absences} 
      ...
      case 'settings':
        return (
          <div style={{ flex: 1, overflow: 'auto', padding: '40px', background: 'white' }}>
